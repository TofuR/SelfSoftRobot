...
def euler_solve(velocity_net, x0, cond, n_steps=50, return_trajectory=False):
    """Euler 法 ODE 积分。

    从 x0 出发，以 dt = 1/n_steps 为步长，
    迭代 x_{t+dt} = x_t + dt * u_theta(x_t, t | cond)。

    Args:
        velocity_net: 可调用对象，签名 (x_t, t, cond) → velocity，形状同 x_t。
...
def rk4_solve(velocity_net, x0, cond, n_steps=20, return_trajectory=False):
    """四阶 Runge-Kutta ODE 积分。

    比 Euler 法精度更高，相同精度下可用更少步数。

    Args:
        velocity_net: 可调用对象，签名 (x_t, t, cond) → velocity。
        x0:    (B, N, 3) 起始点云。
    ...
    for step in range(n_steps):
        ...
        k1 = velocity_net(x, t1, cond)
        k2 = velocity_net(x + dt / 2 * k1, t_half, cond)
        k3 = velocity_net(x + dt / 2 * k2, t_half, cond)
        k4 = velocity_net(x + dt * k3, t_next, cond)
