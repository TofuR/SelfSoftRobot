"""6 通道气压阀控制层 + 自动驱动(自包含移植,不再依赖 real_capture)。
...
- `ValveController`：把 6 维气压向量 `[c0..c5]` 拆成 modbus 两组
  ...
  ModbusManager 在 `modbus.py`(同包,原样保持)。
...
气压单位 kPa，范围 0–500（4–20mA → 0–500kPa，见 modbus.py）。

适配层(设计 spec §4.6):kPa ↔ 模型归一化换算只允许出现在本模块(硬件边界)
与 openloop_planner(优化边界)。`create_valve_controller` 返回本文件内移植的
ValveController;`valve_to_kpa_requested` 把模型归一化动作换算成 kPa。
"""
...
def create_valve_controller(group1: str, group2: str, *,
    ...
    串口 open 在 connect_group 时才发生(阻塞 → 应在后台线程调用)。
    """
