...
- 数据 npz 格式扩展（数组索引，可扩展到任意视角数）：
  ```python
  # npz 中每个 key 对应一个 numpy 数组，通过第 1 维索引视角
  {
      'actions': (N, D),                       # 不变
      'positions': (N, 3, 31),                 # 不变（可选）

      'images': (N, V, H, W),                  # V 个视角的二值图，index 0=front, 1=side, ...
      'depths': (N, V, H, W),                  # V 个视角的深度图，索引与 images 对齐
      'camera_params': (V, param_dim),         # V 个相机的参数，索引与 images 对齐

      'view_names': ['front', 'side', ...],    # 可选：视角名称映射表（用于日志/可视化）
  }
  # 示例：3 视角时
  # images[:, 0, :, :] → front 视角
  # images[:, 1, :, :] → side 视角
