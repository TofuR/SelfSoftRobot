...
### 1.2b 中期目标(2026-07-28 从"非目标"上调)

以下两条原列为非目标,同日决策后改为**中期目标**;第一版仍不做,但设计不得把它们排除在外:

| 项 | 为什么上调 | 对第一版的约束 |
|---|---|---|
| **3D 形态 / 多相机三角化** | 1-DOF 平面运动下"到达目标 + 避障"命题**本身不成立** —— 只有 3D 冗余自由度才有零空间。而单目免标定不可能恢复深度,故状态来源定为**双/多相机标定 + 三角化** | `positions(T,3,N)` schema 保持 3 维(现在被 2D 管线填 0,不要改成 2 维);`scene.dimension` 的 2D 硬断言要留成**可扩展的门**而不是写死;`pc_scale[2] = 1e-6` 的退化保护要标注为"仅 2D 路线成立" |
| **六腔道双段协同** | 后续 6 通道全部气压驱动 | `action_dim` 保持配置项(`models.py:49-50` 已允许 1/3/6);`channel_map` 一律显式,禁止静默补零 |

**连带反转**:`docs/HANDOFF.md` §7.2 不变量 #7"免标定 / `calibrate_cameras.py` 是遗留标定路线"**已作废**,那些代码变为主线基础设施。详见 [`docs/archived/2026-07-28_archive_manifest.md`](../../archived/2026-07-28_archive_manifest.md) §3。
...
> **⚠️ 3D 路线下本节要改(2026-07-28)**:一旦走多相机标定 + 三角化,`camera_pixel → model` **不再是恒等映射**,而是
> ```
> camera_pixel(per-view, V 路) --每视角骨架--> 2D skeletons(V×N×2)
>     --三角化(需外参)--> model(N×3, 度量单位)  --归一化--> model_normalized
> ```
> 连带变化:(a) `camera_pixel` 要带 `view_id`;(b) `registration` 从"单相机位姿漂移检测"扩展为"**每相机内参 + 相机间外参**的漂移检测";(c) `model` 帧从"训练期相机像素"变成**度量 3D**,`mm_per_px = 0.302` 这种 2D 换算不再是主口径;(d) `ndi_mm` 从"仅评价"可升级为标定的辅助约束。
> 第一版(2D)仍按上表执行,但 `frame_id` 枚举要预留 `camera_pixel:<view_id>` 与 `world_mm` 两个取值,不要写死成只有 4 个。
...
> **⚠️ 3D 路线下第 1 条会改(2026-07-28)**:走多相机三角化后,状态是**度量 3D**,障碍可以是真 3D keep-out 体,"平面近似"这条诚实边界**可以取消**(改为"三角化残差 + 标定残差"这一组新的诚实边界)。但**在标定与三角化真正落地之前不得提前取消** —— 第一版仍必须显示 `planar approx`。
> 同理"阀无压力反馈"与"warmup 期间不可信"两条与维度无关,3D 下继续成立。
...
1. **mask 源:训练用 SAM2,在线跑 SAM2 前向流式**(2026-07-28 裁决,**取代本条原先的 white_on_blue 方案**)
   - 原方案是"训练也改用 white_on_blue 单帧阈值,SAM2 只作离线 QC",理由是 SAM2 分块 200 帧 + **双向**传播不可实时。裁决改为保留 SAM2 的 mask 质量(同一 80 px 离群判据下,离群帧 raw **98** / repaired 32 / SAM2 **3**,共 10214 帧),代价是必须自己解决在线化。
   - **因此新增三项必须做的事**(否则在线锚点会落在训练分布外):
     a. 实测 SAM2 **前向流式**单帧 GPU 延迟,与采集节拍(0.2 s)对比;超预算则降节拍或降分辨率,不得默认"够快"。
     b. **量化前向流式 vs 训练用的双向分块的骨架 px 差** —— 训练时锚帧来自"启发式修复的干净帧"(`masks_repaired`),**在线没有这个来源**,必须定义在线锚帧策略(首帧人工确认?质量门控自动选?)并量化其影响。
     c. `white_on_blue` **不废弃**,降级为:SAM2 的在线锚帧来源候选 + 兜底 + 离线 QC 对照。`perception/segmentation.py` 因此仍然是主线代码。
   - 采集时仍须主动压低坏帧率(固定背景、避免手入画、稳定光照)—— 这对两种 mask 源都成立。
...
### 8.1 3D / 6-DOF 采集的追加要求(2026-07-28 新增,第二轮采集用)

第一版(1-DOF 2D)按上面 8 条执行即可。走 3D 多自由度那一轮时**追加**:

| # | 要求 | 依据 / 现成能力 |
|---|---|---|
| 9 | **多相机同步 + 标定**:≥2 路 RealSense,拍标定板得内参 + 外参,标定文件按 `src/calibration/camera_params_format.py` 的约定落盘;标定残差入 manifest | `real_capture` 已支持 1–8 相机同步(`--camera-count` / `--camera-serials`);`scripts/real/calibrate_cameras.py` 与 `src/utils/camera_system.py` 现成 |
| 10 | **三角化产 3D 骨架**:每视角先出 2D 骨架(即 `perception/skeleton.py`,路线无关),再三角化 → `positions(T,3,N)` 填**真实 z** | `src/data/real/triangulation.py` 现成,且它本来就吃 2D 骨架 |
| 11 | **重算归一化**:`pc_scale[2]` 不再是 `1e-6` 退化值。**旧 checkpoint 的 buffer 是 1e-6,与 3D 数据混用会静默出错**,必须新训 | `dataset_spatial.py:139-140` |
| 12 | **6 通道激励协议**(不是把 `action_dim` 从 1 改成 6 就完事):单腔道正交基 → 同段双/三腔道 → 分段单独 → 两段协同 → 动作反转 → 循环加载 → **测试集留未见过的腔道组合** | 实验方案 §7 已写全;`ValveDriver` 现在是逐通道有界随机游走,**6 维随机游走会大量落在无信息区**,需要设计序列而不是加采样量 |
| 13 | **按序列划分 train/val/test,且 test 含未见组合** | 随机帧划分测不出组合泛化 |
| 14 | **标定漂移检测扩展**:`registration.py` 从"单相机位姿"扩展为"每相机内参 + 相机间外参" | 见 §3.3 的 3D 注记 |

**顺序约束**:6 通道数据必须在标定与三角化打通**之后**采 —— 否则只能得到 6-DOF 动作 + 2D 投影状态,而 6 通道驱动下臂会明显离开平面,单目投影会出现自遮挡与深度歧义,那批数据对 3D 目标基本不可用。
