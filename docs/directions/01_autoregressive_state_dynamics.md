...
**失效场景**：
- 时间间隔极长，系统总在平衡态（此时退化为假设一）
- 没有前一状态可用（冷启动）

### 两种假设的关系

两种假设不是矛盾的，而是同一物理过程在不同时间尺度下的近似：

```
假设二（状态转移）在 t→∞ 时 → 退化为假设一（稳态映射）
```

**关键问题：我们的数据采集和实验设计，到底满足哪种假设的条件？**

如果 PyElastica 仿真中每个动作之间间隔足够长（充分松弛），那么假设一成立，当前方法合理。但如果：
- 数据采集时动作变化较快
- 实际控制中需要连续、快速的动作调整
- 材料松弛时间 > 动作间隔
