...
**你已有**：硅胶臂 + 单腔道驱动器（能用）、Intel RealSense D400、**TwinCAT PLC（pyads，`192.168.50.56.1.1:851`）控电机推注射器**（电机位置 mm = 真实控制量）、**Arduino 读 I2C 气压传感器**（COM4@9600，只读不控任何阀）、**NDI 6DOF tracker**（末端度量验证，`ndi.csv: t_sec,x,y,z mm`）。采集程序见 [`docs/ref/Main UI-plc/`](../ref/Main%20UI-plc/)（`dataset_recorder.py` / `main_capture.py`）。
...
| 气压传感（若没有） | I2C 气压传感器（Arduino 只读） | 见下"量程注意" |
| ~~阀/泵~~（已不需） | ~~电磁阀 + MOSFET + 气泵~~ | **作废**：驱动已改为 TwinCAT PLC 控电机推注射器，无气路阀控；Arduino 只读气压 |
| 安全 | 手动断电/卸压阀 | 软件失控时手动停电机或卸压 |
...
5. **接驱动/气路**：**TwinCAT PLC 控电机推注射器**（电机位置 mm = 控制量，手动算好零位/最大弯曲对应行程），注射器出气经 I2C 气压传感器（Arduino 只读）到软臂腔道。传感器**靠近臂**才测的是臂实际受压；**没有电磁阀/气泵**，软件失控时手动停电机。
...
> **采集脚本已存在**（见 [`docs/ref/Main UI-plc/`](../ref/Main%20UI-plc/)：`dataset_recorder.py` headless / `main_capture.py` GUI / `realsense_cam.py`），时间同步 相机+I2C 气压+电机位置，直接产出本节 raw 目录 → 喂 `capture_to_npz`。**实际采集用这套**；下面只留单进程同步的参考逻辑（理解原理 / 改写时参考）。
...
**Arduino 侧**（参考）：经 I2C 读气压传感器 → kPa（COM4@9600，输出 `P:..T:..`）；**上电后气路常压读 N 次取均值当 offset，之后读数减 offset（清零，消大气压偏置）**。Arduino **只读气压**，不控任何阀——驱动由 TwinCAT PLC 独立控电机。
...
**单相机阶段怎么测**（还没有 3D GT）：用** 2D 骨架末端像素坐标**当形状反馈。施加一个阶跃电机位置指令（TwinCAT PLC 突然推到目标行程），逐帧记末端 (x,y)，画时间曲线，**到 95% 稳定的时间 = τ_settle**。
...
> - 可用传感：Intel RealSense D400（当 RGB 用，深度流作离线 GT 地板）+ 手机/普通相机 + 1–3 台工业/USB 相机 + **NDI 6DOF tracker**（末端独立度量验证）。
