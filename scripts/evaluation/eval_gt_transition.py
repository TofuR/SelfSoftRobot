...
def main():
    ...
    print(f"  部署指标（最后一步 ŝ_t，无 loss 时的预测精度）: final MSE = {pm[-1]:.6e}")
    print(f"  per-step MSE（诊断，每步都有 GT）: mean={pm.mean():.6e}, final={pm[-1]:.6e}, max={pm.max():.6e}")
    print(f"  z norm       : start={zn[0]:.4f}, mean={zn.mean():.4f}, final={zn[-1]:.4f}")
    print(f"  z drift ratio (final/start) = {z_drift:.2f}x")
    print(f"{'='*50}")
    print("\n解读（全 GT 驱动窗口框架，s 每步真实）:")
    print("  - 部署时只关心最后一步 ŝ_t（当前预测）；无 GT 时不算 loss，直接用预测")
    print("  - per-step MSE 是训练 dense supervision 的诊断，部署不依赖它")
