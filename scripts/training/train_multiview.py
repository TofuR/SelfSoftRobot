"""train_multiview.py -- Multi-view + depth supervision training via UnifiedTrainer.

Usage:
    # MSTNF multi-view training
    CUDA_VISIBLE_DEVICES=0 python scripts/training/train_multiview.py \
        --model mstnf --data_dir data/seq_rz_c2_sk

    # CMSTNF multi-view training (with depth)
    CUDA_VISIBLE_DEVICES=0 python scripts/training/train_multiview.py \
...
def main():
    parser = argparse.ArgumentParser(description="Multi-view + depth training")
    ...
    # -- ViewStrategy: MultiView --
    view_strat = MultiViewStrategy(
        cam_system, with_depth=args.depth,
        with_consistency=False, with_reprojection=False)
