"""train_multiview.py — 多视角 + 深度监督训练入口（方案 A）。
...
def train(args):
    ...
    # ── 训练器 ──
    trainer = MultiViewTrainer(model, cam_system, device, config=config)
    ...
    exp_dir = create_experiment(
        os.path.join("train_log", f"train_multiview_{args.model}"), config_dict)
    ...
    for epoch in range(1, n_epochs + 1):
        model.train()
        epoch_loss = 0
        epoch_details = {"recon": 0.0, "depth": 0.0, "smooth": 0.0}
        n_batches = 0
...
def main():
    parser = argparse.ArgumentParser(description="多视角+深度监督训练 (方案A)")
