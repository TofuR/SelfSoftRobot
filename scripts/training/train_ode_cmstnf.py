...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 python scripts/training/train_ode_cmstnf.py

    # 仅 Phase 1（canonical field）
    CUDA_VISIBLE_DEVICES=0 python scripts/training/train_ode_cmstnf.py --phase 1

    # 仅 Phase 2（deformation field）
    CUDA_VISIBLE_DEVICES=0 python scripts/training/train_ode_cmstnf.py --phase 2 \
...
trainer = ODECMSTNFTrainer(device=device, config=config)
